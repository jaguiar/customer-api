...
class CustomerResponseTest {
    ...
    fun `CustomerResponse toString should display "SET" for personal information when all customer info present`() {
        ...
        // Assert
        assertThat(toString).isEqualTo(
            "customerId:samsam, firstName:SET, lastName:SET, age:SET, phoneNumber:SET, email:SET, " +
                    "loyaltyProgram: number:SamSaucer, label:Samnounours, " +
                    "validityStartDate:2020-01-01, validityEndDate:2020-12-31, " +
                    "railPasses:[number:gouzilli, label:a ions, validityStartDate:2019-01-01, validityEndDate:2021-01-01]"
        )
    }
}