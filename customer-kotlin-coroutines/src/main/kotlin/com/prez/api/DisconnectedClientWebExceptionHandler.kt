...
class DisconnectedClientWebExceptionHandler : WebExceptionHandler {
    ...
    override fun handle(exchange: ServerWebExchange, ex: Throwable): Mono<Void> {
        ...
        return when {
            response.isCommitted && isDisconnectedClientError(ex) -> {
                ...
                LOGGER.error("Error for ${requestQuery} but ServerHttpResponse already committed ( ${response.statusCode} )", ex)
    ...
    private val disconnectedClientExceptions = setOf("AbortedException", "ClientAbortException", "EOFException", "EofException")
