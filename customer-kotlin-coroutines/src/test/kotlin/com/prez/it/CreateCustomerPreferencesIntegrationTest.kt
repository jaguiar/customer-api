...
class CreateCustomerPreferencesIntegrationTest(
  ...
  @Test
  fun `should return Bad Request when malformed request`() {
    // Given
    val validToken: String = fakeTokenGenerator.generateNotExpiredSignedToken("cached", 3600, "customer.write")
  ...
  fun `should return Bad Request when invalid input values`() {
    ...
    client
      ...
      .expectBody()
      .jsonPath("$.code").isEqualTo("VALIDATION_ERROR")
      .jsonPath("$.message").value(Matchers.containsString("3 error(s) while validating com.prez.api.dto.CreateCustomerPreferencesRequest : "))
